...
function GetInstance (sprite_def)
    ...
    return {
        ...
        curr_frame = 1,
        elapsed_time = 0,
        size_scaleW = scale.wid,
        size_scaleH = scale.hei,
        time_scale = 1.0,
        rotation = 0,
        flip_h = 1,
        flip_v = 1
...
function UpdateInstance(spr, dt, pause)
    ...
    if spr.elapsed_time > spr.sprite.frame_duration * spr.time_scale then
        ...
        if spr.curr_frame <	#spr.sprite.animations[spr.curr_anim] and not pause then
...
function DrawInstance (spr, x, y, dir)
